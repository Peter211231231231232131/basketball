...
export class Game {
    ...
    init() {
        ...
        this.socket.on('player_left', (id) => {
            ...
            if (this.remoteVisuals[id]) {
                this.scene.remove(this.remoteVisuals[id]);
                delete this.remoteVisuals[id];
            }
        });
        ...
        this.socket.on('world_state', (state) => {
            ...
            // Sync Ball (PHYSICS FIX)
            // Only strictly enforce server position if someone owns it.
            // If it's free, we prefer local physics + simple corrections, 
            // OR we fix the server to actually simulate physics (which we can't do here).
            // Current Fix: If server says owner is NULL, we ignore position updates 
            // to prevent "Sky Freeze", relying on the initial Throw velocity to drive it locally.
            // This desyncs clients slightly but makes it playable.
            if (state.ballState) {
                if (state.ballState.ownerId) {
                    // It is held by someone, snap to it
                    this.ball.mesh.position.copy(state.ballState.position);
                    this.ball.velocity.set(0, 0, 0);
                    this.ball.owner = state.ballState.ownerId; // (Need to resolve entity lookup ideally)
                } else {
                    // It's free. 
                    this.ball.owner = null;
                    // Only snap if WAY off (teleport) or velocity is zero?
                    // For now, let's IGNORE server pos loops to fix "Sky Freeze"
                }
            }
    ...
    animate() {
        ...
        // Update Remote Players & Manage Visuals
